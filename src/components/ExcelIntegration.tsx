...
interface ExcelIntegrationProps {
  ...
  onImport: (
    tasks: Task[],
    newCustomFields?: { id: string; name: string; type: any }[],
  ) => Promise<void>;
  open: boolean;
...
export function ExcelImportDialog({
  ...
  const [step, setStep] = React.useState<
    "upload" | "mapping" | "options" | "preview"
  >("upload");
  const [mapping, setMapping] = React.useState<Record<string, string>>({});

  // ⚡ Bolt: Replace O(N) array search inside mapping loop with O(1) map lookup
  // Impact: Reduces O(N*M) complexity when rendering headers and mappings to O(M)
  const customFieldByNameMap = React.useMemo(() => {
    return new Map(
      (space.customFields || []).map((f) => [f.name.toLowerCase(), f]),
    );
  }, [space.customFields]);

  const customFieldByIdMap = React.useMemo(() => {
    return new Map(
      (space.customFields || []).map((f) => [`custom_${f.id}`, f]),
    );
  }, [space.customFields]);

  const customFieldByRawIdMap = React.useMemo(() => {
    return new Map((space.customFields || []).map((f) => [f.id, f]));
  }, [space.customFields]);

  const standardFieldByIdMap = React.useMemo(() => {
    return new Map(STANDARD_FIELDS.map((f) => [f.id, f]));
  }, []);

  const [extraFieldTypes, setExtraFieldTypes] = React.useState<
    Record<string, any>
  >({});

  // ⚡ Bolt: Replace O(N) array search inside mapping loop with O(1) map lookup
  // Impact: Reduces O(N*M) complexity when rendering headers and mappings to O(M)
  const [optionMappings, setOptionMappings] = React.useState<
  ...
  const [constantMappings, setConstantMappings] = React.useState<
    {
      id: string;
      field: string;
      type: "constant" | "date_plus_rowid";
      value: string;
    }[]
  >([]);
    ...
    reader.onload = (evt) => {
      ...
      if (json.length > 0) {
        ...
        h.forEach((header) => {
          ...
          if (lower === "title" || lower === "task" || lower === "name")
            initialMapping[header] = "title";
          else if (lower === "description" || lower === "desc")
            ...
            initialMapping[header] = "assignee";
          else if (lower === "duedate" || lower === "due")
            initialMapping[header] = "dueDate";
          else if (lower === "priority") initialMapping[header] = "priority";
          else {
            // Check custom fields
            const cf = customFieldByNameMap.get(header.toLowerCase());
      ...
      Object.entries(mapping).forEach(([header, target]) => {
        if (target === "extra") {
          ...
          newCustomFieldsToCreate.push({
            id: newId,
            name: header,
            type: extraFieldTypes[header] || "text",
          });
        }
      });
      ...
      const tasksToImport: Task[] = data.map((row, rowIndex) => {
        ...
        headers.forEach((h, i) => {
          ...
          if (mapTo === "title") task.title = String(val);
            ...
            const lookupVal = mappedVal
              ? String(mappedVal).toLowerCase()
              : String(val).toLowerCase();
            const col =
              statusById.get(lookupVal) || statusByNameLower.get(lookupVal);
            ...
            const user =
              userById.get(lookupVal) ||
              userByNameLower.get(lookupValLower) ||
              userByEmailLower.get(lookupValLower);
                ...
                if (!isNaN(parsed.getTime())) {
                  ...
                  if (parts.length === 3) {
                    ...
                    if (parts[0] > 1000) {
                      ...
                      if (parts[1] > 12) {
                        // must be MM-DD-YYYY? No, wait, if m > 12 it's invalid unless it's DD.
            ...
            const mappedVal =
              optionMappings[h]?.[String(val)] || String(val).toLowerCase();
            if (["high", "medium", "low"].includes(mappedVal))
              task.priority = mappedVal;
            ...
            if (customField?.type === "select") {
              task.custom[fieldId] =
                optionMappings[h]?.[String(val)] || String(val);
            } else {
  ...
  const prepareOptionsStep = () => {
    ...
    const newOptionMappings: Record<string, Record<string, string>> = {
      ...optionMappings,
    };
    ...
    headers.forEach((h, i) => {
      ...
      if (
        ...
        mapTo === "priority" ||
        mapTo === "assignee" ||
        mapTo.startsWith("custom_")
      ) {
        let isSelect =
          mapTo === "status" || mapTo === "priority" || mapTo === "assignee";
        ...
        if (isSelect) {
          const values = Array.from(
            new Set(data.map((row) => String(row[i] || ""))),
          ).filter(Boolean);
          ...
          values.forEach((val) => {
            if (!newOptionMappings[h][val]) {
              ...
              if (mapTo === "status") {
                ...
                if (["high", "medium", "low"].includes(valLower))
                  newOptionMappings[h][val] = valLower;
              } else if (mapTo === "assignee") {
          ...
          {step === "mapping" && (
            <div className="space-y-4 flex flex-col flex-1 overflow-hidden">
              <div className="flex items-start gap-3 p-3 bg-primary/5 rounded-lg border border-primary/10">
                ...
                <p className="text-xs text-muted-foreground">
                  Match the columns from your file to the fields in SyncDuo.
                  Extra columns can be imported as new custom fields.
                </p>
              </div>
                  ...
                  {headers.map((header) => (
                    <React.Fragment key={header}>
                      ...
                      {mapping[header] === "extra" ? (
                        <Select
                          value={extraFieldTypes[header]}
                          onValueChange={(v) =>
                            setExtraFieldTypes((prev) => ({
                              ...prev,
                              [header]: v,
                            }))
                          }
                ...
                {constantMappings.length > 0 && (
                  <div className="grid grid-cols-[1fr_auto_1fr_auto] gap-x-4 gap-y-2">
                    ...
                    {constantMappings.map((c, idx) => {
                      const isDate =
                        c.field === "dueDate" ||
                        customFieldByIdMap.get(c.field)?.type === "date";
                      return (
                        <React.Fragment key={c.id}>
                          <Select
                            ...
                            onValueChange={(val) => {
                              ...
                              if (
                                val !== "dueDate" &&
                                customFieldByIdMap.get(val)?.type !== "date"
                              ) {
                          ...
                          {isDate ? (
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="date_plus_rowid">
                                  Base + Row ID
                                </SelectItem>
                              </SelectContent>
                            </Select>
                          ...
                          {(() => {
                            ...
                            const customField = customFieldByIdMap.get(c.field);
                            if (customField?.type === "select") {
                              return (
                                <Select
                                  value={c.value}
                            ...
                            return (
                              <Input
                                placeholder={
                                  isDate && c.type === "date_plus_rowid"
                                    ? "Base Date (e.g., 2024-01-01)"
                                    : "Value"
                                }
                                value={c.value}
                                onChange={(e) => {
          ...
          {step === "options" && (
            <div className="space-y-4 flex flex-col flex-1 overflow-hidden">
              <div className="flex items-start gap-3 p-3 bg-primary/5 rounded-lg border border-primary/10">
                <AlertCircle className="size-5 text-primary shrink-0 mt-0.5" />
                <p className="text-xs text-muted-foreground">
                  Map values from your Excel columns to the available options in
                  SyncDuo.
                </p>
              </div>
                ...
                <div className="space-y-6 pr-4">
                  {Object.entries(uniqueValues).map(([header, values]) => {
                    ...
                    if (mapTo === "status")
                      ...
                      const field = customFieldByRawIdMap.get(
                        mapTo.replace("custom_", ""),
                      );
          ...
          {step === "preview" && (
            <div className="flex flex-col flex-1 overflow-hidden space-y-4">
                  ...
                  <TableHeader>
                    <TableRow>
                      ...
                      {constantMappings.map((c) => {
                        const fieldName =
                          standardFieldByIdMap.get(c.field)?.label ||
                          customFieldByIdMap.get(c.field)?.name ||
                          c.field;
                        return (
                          <TableHead key={c.id}>[Const] {fieldName}</TableHead>
                        );
                      })}
                    </TableRow>
                  </TableHeader>
                      ...
                      <TableRow key={i}>
                        ...
                        {constantMappings.map((c) => {
                          ...
                          return (
                            <TableCell
                              key={c.id}
                              className="text-xs max-w-[200px] truncate italic text-muted-foreground"
                            >
                              {val}
                            </TableCell>
                          );
              ...
              <p className="text-[10px] text-muted-foreground">
                Showing first 10 rows. Total rows to import: {data.length}
              </p>
