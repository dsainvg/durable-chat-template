...
export function GlobalAutomationsDialog({
  ...
  const handleAddCondition = () => {
    setConditions([
      ...
      {
        type: "task_field",
        config: { field: "status", operator: "equals", value: "" },
      },
    ]);
  };
  ...
  const updateCondition = (
    index: number,
    val: Partial<AutomationCondition>,
  ) => {
  ...
  const spaceNameMap = useMemo(
    () => Object.fromEntries(state.spaces.map((s) => [s.id, s.name])),
    [state.spaces],
  );
            ...
            <section>
              ...
              {automations.length === 0 ? (
                <p className="text-sm text-muted-foreground italic">
                  No global automations configured.
                </p>
              ) : (
                <div className="space-y-2">
                  {automations.map((a) => (
                    <div
                      key={a.id}
                      className="bg-card border border-border p-3 rounded-md text-sm flex justify-between items-start gap-4"
                    >
                      <div className="space-y-1">
                        <p className="font-medium text-xs text-muted-foreground">
                          Targets:{" "}
                          {a.targetSpaces.length === 0
                            ? "Any Space"
                            : a.targetSpaces
                                .map((sid) => spaceNameMap[sid] || sid)
                                .join(", ")}
                        </p>
                        <div>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This action cannot be undone. This will permanently delete the global automation.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
            ...
            <section className="bg-secondary/30 p-4 rounded-xl border border-border space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-xs font-semibold">
                  1. Target Spaces (Leave empty for ALL spaces)
                </Label>
                <div className="flex flex-wrap gap-2">
                  {state.spaces.map((s) => (
                    <label
                      key={s.id}
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border text-xs cursor-pointer transition-colors ${targetSpaces.includes(s.id) ? "bg-primary/20 border-primary" : "bg-background border-border hover:bg-muted"}`}
                    >
                      <Checkbox
                        checked={targetSpaces.includes(s.id)}
                        onCheckedChange={() => toggleTargetSpace(s.id)}
                      />
                      {s.name}
                    </label>
                  ))}
              ...
              <div className="space-y-2">
                ...
                {conditions.map((c, i) => {
                  const isLegacy =
                    !c.type.startsWith("task_field") &&
                    !c.type.startsWith("space_activity");
                          ...
                          <>
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="has_assignee">
                                  Has Assignee
                                </SelectItem>
                                <SelectItem value="no_assignee">
                                  No Assignee
                                </SelectItem>
                                <SelectItem value="status_equals">
                                  Status Equals...
                                </SelectItem>
                                <SelectItem value="status_not_equals">
                                  Status Not Equals...
                                </SelectItem>
                                <SelectItem value="priority_equals">
                                  Priority Equals...
                                </SelectItem>
                                <SelectItem value="priority_not_equals">
                                  Priority Not Equals...
                                </SelectItem>
                                <SelectItem value="due_date_equals">
                                  Due Date Equals...
                                </SelectItem>
                                <SelectItem value="assignee_equals">
                                  Assignee Equals...
                                </SelectItem>
                                <SelectItem value="no_new_tasks_created">
                                  No New Tasks Created
                                </SelectItem>
                                <SelectItem value="no_new_tasks_in_status">
                                  No New Tasks In Status...
                                </SelectItem>
                                <SelectItem value="no_new_tasks_by_user_in_status">
                                  No New Tasks By User In Status...
                                </SelectItem>
                                <SelectItem value="no_new_tasks_in_priority">
                                  No New Tasks In Priority...
                                </SelectItem>
                                <SelectItem value="no_new_tasks_by_user_in_priority">
                                  No New Tasks By User In Priority...
                                </SelectItem>
                                <SelectItem value="task_field">
                                  Task Property...
                                </SelectItem>
                                <SelectItem value="space_activity">
                                  Space Activity...
                                </SelectItem>
                              </SelectContent>
                            </Select>
                            {(c.type === "status_equals" ||
                              c.type === "status_not_equals" ||
                              c.type === "no_new_tasks_in_status") && (
                              <Select
                                value={c.config?.status || ""}
                                onValueChange={(v) =>
                                  updateCondition(i, {
                                    config: { ...c.config, status: v },
                                  })
                                }
                              >
                                <SelectTrigger className="h-8 text-xs">
                            ...
                            {(c.type === "priority_equals" ||
                              c.type === "priority_not_equals" ||
                              c.type === "no_new_tasks_in_priority") && (
                              <Select
                                value={c.config?.priority || "low"}
                                onValueChange={(v) =>
                                  updateCondition(i, {
                                    config: { ...c.config, priority: v },
                                  })
                                }
                              >
                                <SelectTrigger className="h-8 text-xs">
                            ...
                            {c.type === "assignee_equals" && (
                              <Select
                                value={c.config?.assignee || ""}
                                onValueChange={(v) =>
                                  updateCondition(i, {
                                    config: { ...c.config, assignee: v },
                                  })
                                }
                              >
                                <SelectTrigger className="h-8 text-xs">
                            ...
                            {c.type === "no_new_tasks_by_user_in_status" && (
                              <div className="flex gap-2 w-full mt-2">
                                <Select
                                  value={c.config?.user_id || ""}
                                  onValueChange={(v) =>
                                    updateCondition(i, {
                                      config: { ...c.config, user_id: v },
                                    })
                                  }
                                >
                                  <SelectTrigger className="h-8 text-xs">
                          ...
                          <div className="space-y-2">
                            ...
                            {c.type === "task_field" && (
                              <div className="flex gap-2 w-full mt-2">
                                ...
                                <Select
                                  value={c.config?.operator || "equals"}
                                  onValueChange={(v) =>
                                    updateCondition(i, {
                                      config: { ...c.config, operator: v },
                                    })
                                  }
                                >
                                  <SelectTrigger className="h-8 text-xs flex-1">
                                  ...
                                  <SelectContent>
                                    ...
                                    {c.config?.field === "due_date" && (
                                      <>
                                        <SelectItem value="is_today">
                                          Is Today
                                        </SelectItem>
                                        <SelectItem value="is_overdue">
                                          Is Overdue
                                        </SelectItem>
                                      </>
                                    )}
                                ...
                                {![
                                  ...
                                  "is_today",
                                  "is_overdue",
                                ].includes(c.config?.operator || "equals") && (
                            ...
                            {c.type === "space_activity" && (
                              <div className="flex gap-2 w-full mt-2">
                                <Select
                                  value={c.config?.event || "no_created"}
                                  onValueChange={(v) =>
                                    updateCondition(i, {
                                      config: {
                                        ...c.config,
                                ...
                                <Select
                                  value={c.config?.user || "any"}
                                  onValueChange={(v) =>
                                    updateCondition(i, {
                                      config: { ...c.config, user: v },
                                    })
                                  }
                                >
                                  <SelectTrigger className="h-8 text-xs flex-1">
                                ...
                                {["no_status", "no_priority"].includes(
                                  c.config?.event,
                                ) && (
                                  <div className="flex-1">
              ...
              <div className="space-y-2 pt-2 border-t border-border">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    if (v === "send_email")
                      ...
                      setActionConfig({ new_status: "done" });
                    else if (v === "move_space")
                      setActionConfig({
                        new_space_id: state.spaces[0]?.id || "",
                      });
                  }}
                >
                  <SelectTrigger>
                ...
                {actionType === "send_email" && (
                  <div className="mt-2 space-y-1">
                    ...
                    <Select
                      value={actionConfig.target_user_id}
                      onValueChange={(v) =>
                        setActionConfig({ ...actionConfig, target_user_id: v })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                ...
                {actionType === "change_status" && (
                  <div className="mt-2 space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setActionConfig({ ...actionConfig, new_status: v })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select Status" />
                ...
                {actionType === "move_space" && (
                  <div className="mt-2 space-y-1">
                    ...
                    <Select
                      value={actionConfig.new_space_id}
                      onValueChange={(v) =>
                        setActionConfig({ ...actionConfig, new_space_id: v })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
              ...
              <div className="space-y-2 pt-2 border-t border-border">
                <Label className="text-xs font-semibold">
                  4. Execution Time (Optional)
                </Label>
                <div className="space-y-1">
                  <Select
                    value={actionConfig.run_time || "immediate"}
                    onValueChange={(v) =>
                      setActionConfig({
                        ...actionConfig,
                        run_time: v === "immediate" ? undefined : v,
                      })
                    }
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="immediate">
                        ⚡ Run immediately (on every event check)
                      </SelectItem>
                  ...
                  <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                    <span>ℹ️</span> Evaluates this rule only at a specific
                    30-minute tick between 07:00 and 23:30.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                >
                  Repeat daily if conditions are met
                </Label>
              </div>
